...
def display_content():
    ...
    st.markdown("""
                    ...
                    ● Si vous téléversez ou générez aléatoirement un jeu de données, puis naviguez vers une autre page et revenez à la page d'analyse, tous les résultats d'analyse statistique seront supprimés. Pour les réafficher, il vous suffit de réinsérer la proportion ou la moyenne hypothétique de la population, de sélectionner la catégorie visée dans l'analyse (la catégorie de la variable catégorielle binaire qui reflète le succès ou la valeur 1 dans votre contexte) et de cliquer sur le bouton 'Analyser'. Aucun re-téléversement des données n'est nécessaire pour refaire l'analyse statistique.
